...
import { Play, SkipForward, Edit3, RefreshCw, AlertCircle, CheckCircle, Clock, ExternalLink } from 'lucide-react';

// Auto-detect API URL - will use the actual Vercel deployment
const API_BASE_URL = process.env.REACT_APP_API_URL || 
  process.env.NODE_ENV === 'production' 
    ? 'https://company-naming-workflow-backend.vercel.app/api/workflow'
    : 'http://localhost:3001/api/workflow';

const CompanyNamingWorkflow = () => {
  ...
  // Auto-detect deployment URL and check API health
  useEffect(() => {
    const detectedURL = API_BASE_URL.replace('/api/workflow', '');
  ...
  const checkAPIHealth = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setSuccess(`✅ Connected to live API: ${deploymentURL}`);
  ...
  const executeStep = async (stepIndex) => {
    ...
    try {
      ...
      console.log(`Executing Step ${step.id} with ${step.llm}...`);
      ...
      if (result.success) {
        ...
        if (step.id === 1) {
          ...
          if (sectionAMatch) newOutputs[`STEP_${step.id}_A`] = sectionAMatch[1].trim();
          if (sectionBMatch) newOutputs[`STEP_${step.id}_B`] = sectionBMatch[1].trim();
        }
        
        setStepOutputs(prev => ({ ...prev, ...newOutputs }));
        setSuccess(`✅ Step ${step.id} completed! ${step.llm} generated ${result.output.length} characters in ${executionTime}ms`);
      ...
      setError(`❌ Step ${stepIndex + 1} failed: ${error.message}`);
    } finally {
      setIsRunning(false);
    }
  };
  ...
  const runAllSteps = async () => {
    ...
    try {
      ...
      setSuccess(`🚀 Full workflow started! Execution ID: ${result.executionId}`);
  ...
  const pollWorkflowStatus = async (id) => {
    try {
      ...
      Object.entries(execution.results).forEach(([key, stepResult]) => {
        if (stepResult.success) {
          ...
          if (stepId === 1) {
            ...
            if (sectionAMatch) newOutputs[`STEP_${stepId}_A`] = sectionAMatch[1].trim();
            if (sectionBMatch) newOutputs[`STEP_${stepId}_B`] = sectionBMatch[1].trim();
          }
        }
      });
      ...
      if (execution.status === 'completed') {
        setIsAutoRunning(false);
        setSuccess('🎉 Complete workflow finished! All 9 steps generated comprehensive naming strategy.');
      } else if (execution.status === 'error') {
        setError(`❌ Workflow failed: ${execution.error}`);
        setIsAutoRunning(false);
      } else {
        setSuccess(`⏳ Processing Step ${execution.currentStep}/${execution.totalSteps}...`);
  ...
  const APIStatusIndicator = () => (
    <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm ${
      ...
      <span className="font-medium">
        {apiStatus === 'healthy' ? 'Live API Connected' : 
         apiStatus === 'error' ? 'API Disconnected' : 'Connecting...'}
      </span>
      {apiStatus === 'healthy' && deploymentURL && (
        <a 
          href={deploymentURL} 
          target="_blank" 
          rel="noopener noreferrer"
          className="text-xs bg-green-100 px-2 py-1 rounded flex items-center gap-1 hover:bg-green-200"
        >
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
      <div className="p-6 border-b border-gray-200">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-xl font-bold text-gray-900 mb-2">Company Naming Workflow</h2>
            <p className="text-gray-600">AI-powered 9-step naming strategy with real LLM integrations</p>
          </div>
          <APIStatusIndicator />
        </div>
      </div>
          ...
          <button
            onClick={async () => {
              if (apiStatus !== 'healthy') {
                setError('❌ Please wait for API connection');
                return;
              }
              
              const validation = await validateBusinessData();
              if (!validation.isValid) {
                setError(`❌ Please fill required fields: ${validation.missingFields.join(', ')}`);
                return;
              }
              
              setShowBusinessForm(false);
              setSuccess('✅ Ready to generate your company naming strategy!');
            }}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold">
                    Step {prompts[currentStep]?.id}: {prompts[currentStep]?.title}
                  </h3>
...
export default CompanyNamingWorkflow;